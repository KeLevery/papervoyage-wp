/**
 * 纸旅 PaperVoyage · PJAX 无感切换（WordPress 版）
 *
...
(function () {
	...
	var noCache = document.body.classList.contains('admin-bar');
	var cache = new Map();
	var CACHE_MAX = 10;
	var busy = false;
	...
	/* ---------- body 类迁移（含 cat-{slug} 栏目视觉、页面模板类） ---------- */
	...
	function updateNav(url) {
		...
		$$('.main-navigation a').forEach(function (a) {
			...
			if (a.parentElement) {
				...
				a.parentElement.classList.toggle('current_page_item', hit);
			}
		});
	}
	...
	/* main 之外、body 之内的内联脚本：外链脚本与 speculationrules 跳过，避免重复初始化 */
	...
	/* innerHTML 注入的 <script> 不会执行，逐个重建（覆盖嵌入代码等场景） */
	...
	/* 文章页按需补载 comment-reply（首屏不是文章页时它不会被 enqueue） */
	function ensureCommentReply() {
		if (window.addComment) return;
		if (!document.querySelector('#commentform, #respond form')) return;
		var src = window.papervoyageData && window.papervoyageData.commentReplySrc;
		if (!src) return;
		var s = document.createElement('script');
		s.src = src;
	...
	function navigate(href, push, scrollTop) {
		...
		var cached = noCache ? null : cache.get(key);
